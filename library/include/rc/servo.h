 ...
 * printed in white silkscreen on the cape before plugging anything in. The
 * standard pinnout for these 3 pin connectors is as follows.
 *
 * 1 - Ground
 * 2 - 6V Power
...
int rc_servo_init();
int rc_servo_cleanup();
int rc_servo_power_rail(int en);
 ...
 * rc_disable_servo_power_rail() can be called to turn off power to the servos
 * for example when the robot is in a paused state to save power or prevent
 * noisy servos from buzzing.
 *
* @ int rc_send_servo_pulse_normalized(int ch, float input)
* @ int rc_send_servo_pulse_normalized_all(float input)
*
