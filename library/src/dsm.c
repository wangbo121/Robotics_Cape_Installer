...
int rc_dsm_bind_routine(){
	...
	if(rc_gpio_init(DSM_PIN, GPIOHANDLE_REQUEST_INPUT)){
		fprintf(stderr,"ERROR in rc_dsm_bind_routine initializing gpio pin %d\n", DSM_PIN);
		return -1;
	}
...
enter:
	...
	// configure gpio pin
	if(rc_gpio_init(DSM_PIN, GPIOHANDLE_REQUEST_OUTPUT)){
