 ...
 * and SPI1.2 These share clock and serial IO signals, but have independent
 * slave select lines.
 *
 * The slaves can be selected automatically by the SPI linux driver or manually
 * with select/deselect_spi_slave() functions. On the Robotics Cape, slave 1 can
 * be used in either mode, but slave 2 must be selected manually. On the BB Blue
 * either slave can be used in manual or automatic modes.
 *
