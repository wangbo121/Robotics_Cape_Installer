...
// reference: https://elinux.org/images/9/9b/GPIO_for_Engineers_and_Makers.pdf
...
static int line_fd[N_CHIPS*LINES_PER_CHIP];
...
int rc_gpio_init_output(int pin)
{
	int chip, line;
	struct gpiohandle_request req;

	// convert pin to chip&line, this also does santiy checks
	if(pin_to_chip(pin, &chip, &line)) return -1;

	// open chip if not opened already
	if(chip_fd[chip]==0){
		if(open_gpiochip(chip)) return -1;
	}
	...
	if(ioctl(chip_fd[chip], GPIO_GET_LINEHANDLE_IOCTL, &req)==-1){
		perror("ERROR in rc_gpio_init_output");
		return -1;
	}
	line_fd[pin]=req.fd;
...
int rc_gpio_set_value(int pin, int value)
{
	...
	if(ioctl(line_fd[pin], GPIOHANDLE_SET_LINE_VALUES_IOCTL, &data)){
...
int rc_gpio_init_input(int pin)
{
	int chip, line;
	struct gpiohandle_request req;
...
int rc_gpio_get_value(int pin)
{
	...
	if(ioctl(line_fd[pin], GPIOHANDLE_GET_LINE_VALUES_IOCTL, &data)){
